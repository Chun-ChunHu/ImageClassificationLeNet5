        ...
        for f in range(self.F):
            for w in range(0, WW, self.s):
                for h in range(0, HH, self.s):
                    feature_maps[f,w,h] = np.round(np.sum(self.inputs[:,w:w+self.K,h:h+self.K]*self.weights[f,:,:,:]) + self.bias[f], 16)
...
class Sigmoid:
    ...
    def forward(self, inputs):
        self.cache = inputs
        # origianl formula should return 1.0 / (1.0 + np.exp(-x)). 
        # To avoid overflow, alternative formula of logistic function is written here
        ret = .5 * (1 + np.tanh(.5 * inputs))
        return ret
    def backward(self, dy):
        x = self.cache
        dx = np.float16(dy) * np.float16(x) 
        dx = np.float16(dx) * np.float16(1 - x)
        return dx
    def extract(self):
        return
# sigmoid * x, also known as Swish function
class Swish:
    def __init__(self):
        self.cache = None
        pass
    def trunc(self, a, decimals = 0):
        return np.trunc(a*10**decimals)/(10**decimals)
    def sigmoid(self, x):
        sig = .5 * (1 + np.tanh(.5 * x))
        return sig
    def swish_function(self, x):
        swish = x * self.sigmoid(x)
        return swish
    def forward(self, inputs):
        self.cache = inputs
        swish = self.trunc(inputs * self.sigmoid(inputs), 6)
        return swish
    def backward(self, dy):
        x = self.cache
        swish_der = self.trunc(dy * self.swish_function(x) + self.sigmoid(x) * (1 - (dy * self.swish_function(x))), 6)
        return swish_der
    def extract(self):
        return
...
class Softmax:
    def __init__(self):
        pass
    def forward(self, inputs):
        exp = np.exp(inputs, dtype = np.float16) 
